...
class Kohana_Curl {
     ...
     * @param   bool    $headers_only   flag to return only the headers
     * @param   array   $curl_options   additional curl options to instantiate curl with
	 * @param	bool	$url_encoded	send data with content-type application/x-www-form-urlencoded
     * @return  string  result 
     */
    public static function post($url, Array $data = array(), Array $headers = array(), $headers_only = FALSE, Array $curl_options = array(), $url_encoded = FALSE)
    {
        $ch = Curl::factory($curl_options);
